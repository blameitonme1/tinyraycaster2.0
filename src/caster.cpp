...
void draw_monster(Monster &monster, std::vector<float> &depth_buffer, std::vector<float> &monst_depthBuffer, FramBuffer &fb, Player &player, Texture &tex_monster)
{
    ...
    int h_offset = (monst_dir - player.a) * (fb.w / 2) / (player.sightAngle) + (fb.w) / 2 / 2 - monst_screensize / 2;
    int v_offset = fb.h / 2 - monst_screensize / 2;
    for (size_t i = 0; i < monst_screensize; ++i)
    {
        if (h_offset + int(i) < 0 || h_offset + i >= fb.w / 2)
            continue;
        if (depth_buffer[h_offset + i] < monster_dist)
            continue; // 这个怪物被墙挡住了，不画出来
        if (monst_depthBuffer[h_offset + i] < monster_dist)
            continue;                                   // 这个怪物这一列被其他怪物挡住了，不画出来
...
void render(FramBuffer &fb, GameState &gs)
{
    ...
    for (size_t i = 0; i < fb.w / 2; ++i)
    {
        ...
        for (float t = 0; t < 20; t += 0.01)
        {
            ...
            // 画出视线图
            fb.set_pixel(x * rect_w, y * rect_h, pack_color(190, 190, 190));
            if (map.is_empty(x, y))
                continue;
            ...
            depth_buffer[i] = dist;
            size_t colum_height = std::min(2000, int(fb.h / dist)); // 设置最高为2000
